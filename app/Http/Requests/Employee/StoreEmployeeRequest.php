...
class StoreEmployeeRequest extends FormRequest
{
    ...
    {
        return [
            'document_type_id' => ['required', Rule::exists('catalogs', 'id')->where('type', 'DOCUMENT_TYPE')],
            'document_number' => ['required', 'string', 'max:20', Rule::unique('employees', 'document_number')->withoutTrashed()],
            'first_name' => ['required', 'string', 'max:100'],
            'last_name' => ['required', 'string', 'max:100'],
            'birth_date' => ['nullable', 'date', 'before:today'],
            'gender_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'GENDER')],
            'marital_status_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'MARITAL_STATUS')],
            ...
            'position_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'POSITION')],
            'work_area_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'WORK_AREA')],
            'payroll_group_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'PAYROLL_GROUP')],
            'work_shift_id' => ['nullable', 'exists:work_shifts,id'],
            'employment_status_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'WORKER_STATUS')],
            'base_salary' => ['required', 'numeric', 'min:0', 'max:99999999.99'],
            'pension_system_id' => ['nullable', Rule::exists('catalogs', 'id')->where('type', 'PENSION_SYSTEM')],
