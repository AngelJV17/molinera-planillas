        ...
        <section class="space-y-6">
            <div class="rounded-3xl border border-primary/20 bg-white p-6 shadow-lg">
                <div class="flex gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-black text-gray-900">Usuarios y permisos</h1>
                        <p class="mt-1 text-sm text-gray-600">Administra accesos simulados para gerencia, contador y RRHH.</p>
                    </div>
                </div>
            </div>

            <form class="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-md lg:grid-cols-[1fr_1fr_220px_auto]" @submit.prevent="addUser">
                <input v-model="userForm.nombre" required type="text" placeholder="Nombre"
                    class="rounded-xl border-gray-300 text-sm focus:border-primary focus:ring-primary" />
                <input v-model="userForm.correo" required type="email" placeholder="Correo"
                    class="rounded-xl border-gray-300 text-sm focus:border-primary focus:ring-primary" />
                ...
                <tr v-for="user in users" :key="user.id" class="text-sm transition hover:bg-primary/5">
                    ...
                    <td class="px-6 py-4">
                        <span class="rounded-full px-3 py-1 text-xs font-bold"
                            :class="user.estado === 'Activo' ? 'bg-primary/15 text-primary' : 'bg-secondary/15 text-secondary'">
                            {{ user.estado }}
                        </span>
                    </td>
                </tr>
